...
export default function DashboardPage({ navigate, me }) {
  ...
  return <>
    <PageHeader title="Visão geral" subtitle="Operação comercial, licenciamento e saúde do produto." action={<button className="ghost" onClick={load}>Atualizar</button>} />
    <Alert>{error}</Alert>

    {runtime?.maintenance_mode && <div className="maintenance-banner"><div><strong>Modo manutenção ativo</strong><span>{runtime.maintenance_message || 'O aplicativo está configurado para manutenção.'}</span></div><StatusBadge status="suspended" /></div>}
    ...
    {data && <>
      ...
      <div className="dashboard-grid">
        <Panel>
          <div className="panel-head"><div><h2>Vencem nos próximos 7 dias</h2><p className="muted">Prioridade para cobrança e renovação.</p></div></div>
          {data.expiring_7_days?.length ? <div className="simple-list">{data.expiring_7_days.map(item => {
            const days = daysUntil(item.current_period_end)
            return <button className="list-row button-row" key={item.organization_id} onClick={() => navigate(`/companies/${item.organization_id}`)}><div><strong>{item.company_name}</strong><span>Vence em {dateBR(item.current_period_end)}</span></div><span className={`days-pill ${days != null && days <= 2 ? 'urgent' : ''}`}>{days ?? '—'} dias</span></button>
          })}</div> : <Empty>Nenhuma assinatura vence nos próximos 7 dias.</Empty>}
        </Panel>
        ...
        <Panel>
          ...
          {me?.role === 'owner' && <button className="secondary full" onClick={() => navigate('/system')}>Abrir configurações do sistema</button>}
        </Panel>
      </div>
    </>}
  </>
